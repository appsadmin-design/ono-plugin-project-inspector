...
## Target-root resolution and artifact verification (worktree safety)

The plugin's artifacts (`CLAUDE.md`, `AUDIT.md`, `docs/project/`, `audits/`, `.ono/state.json`) must land in the developer's real repository — never in an ephemeral execution copy. Claude Code may run an agent inside an isolated git worktree at `<repo>/.claude/worktrees/agent-<id>/`; inside it, the current working directory and `git rev-parse --show-toplevel` both resolve to the worktree (a legitimate git root), so any CWD-relative write silently strands artifacts there while completion checks against the same wrong root report success. Two deterministic helpers close that gap:

- `scripts/resolve-repo-root.ts` resolves a single authoritative `TARGET_ROOT` at startup (from `hooks/before-inspect.md`). It unwraps a Claude agent worktree to the main working tree using `git worktree list --porcelain` (the first `worktree` entry is always the main tree). It unwraps **only** `.claude/worktrees/` paths — a worktree the developer deliberately targeted is honored. If a worktree can't be safely unwrapped, it exits non-zero and the workflow stops rather than writing.
- `scripts/verify-artifacts.ts` is the Option-3 backstop each after-hook runs: it independently confirms a stage's produced artifacts exist at `TARGET_ROOT` (never trusting the skill's textual "written" report), fails if `TARGET_ROOT` is itself a worktree, and scans `<TARGET_ROOT>/.claude/worktrees/*/` to detect the specific "leaked into a worktree" failure.

`TARGET_ROOT` is threaded — as an absolute path — into every skill invocation and every `inspection-state` call; `inspection-state.ts` additionally refuses to operate on any `.claude/worktrees/` path. A stage is reported complete only after its `verify-artifacts` check passes at `TARGET_ROOT`. These are defense-in-depth layers: resolve-before-write fixes the cause, and the guards plus independent verification make a false "complete" against a worktree structurally impossible.
